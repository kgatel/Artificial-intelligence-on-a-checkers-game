...
\begin{algorithm}
	...
	\begin{algorithmic}
		\State  Entier res $\leftarrow$ 0;
%		\State booleen ordiBlanc $\leftarrow$ false;
		\If { ((dam.case(x-1,y-1) est une Dame) et (dam.case(x-2,y-2) est vide) ) ou ((dam.case(x+1,y-1) est une Dame) et (dam.case(x+2,y-2))) } // en position de manger (dans le cas où on ne peut pas manger en arrière)
		\State res $\leftarrow$ res +20;
		\EndIf
		\If{(y=1) et ( (dam.case(x-1, 0) est Vide) ou dam.case(x+1,0) est Vide)}
		\State res $\leftarrow$ res + 15 //faire une dame
		\EndIf
		\If {(x=0) ou (x=taille de dam)}
...
\begin{figure}[H]
	\center
	\includegraphics[width=0.8\textwidth]{./Images/Diagramme de séquence tourOrdiIA}
